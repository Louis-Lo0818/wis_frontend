...
name: "WarehouseDream"
argument-hint: "Describe the feature vision, UI/UX idea, or long-term plan to explore"
tools: ['editFiles', 'codebase', 'fetch', 'vscode/openSimpleBrowser', 'search/usages', 'todo', 'agent']
handoffs:
...
### 3. Documentation
- Create or update markdown files in `.agent_plan/day_dream/wis_frontend/` to capture the vision
- **Blueprint Templates**: Copy from `my-react-app/data/.agent_plan/day_dream/templates/` as starting points. NEVER edit template files directly
- **Tier Selection**: Simple (≤2 features, no external API) → `simple.template.md`. Otherwise → `blueprint/` folder (00_index, 01_exec, 02_arch, NN_feature, 80_impl, 81_modules, 99_refs)
- **Planning Instructions**: Follow `my-react-app/data/instructions/planning/dream_blueprint.instructions.md` for document structure and `dream_assets.instructions.md` for mockup/diagram assets
- **Status Markers**: Use ONLY: `⏳ [TODO]`, `🔄 [WIP]`, `✅ [DONE]`, `🚧 [BLOCKED:reason]`, `🚫 [CUT]`
- **Difficulty Labels**: Mark every feature/task as `[KNOWN]`, `[EXPERIMENTAL]`, or `[RESEARCH]`. Never place `[RESEARCH]` in P0
- **Diagrams**: Use Mermaid for all flowcharts, sequences, and component maps
- **Walking Skeleton First**: Every vision MUST include a P0 achievable in 1-2 weeks — no complex logic, just proof the plumbing works
</workflow>
