...
export function generateProductsCSV(products: Product[]): string {
  ...
  const rows = products.map(p => `${escapeCSV(p.code)},${escapeCSV(p.name)},${p.weight}`).join('\n');
  return header + rows;
}

export function generateInventoryCSV(inventory: InventoryItem[]): string {
  const header = 'productCode,location,quantity\n';
  const rows = inventory.map(i => `${escapeCSV(i.productCode)},${escapeCSV(i.location)},${i.quantity}`).join('\n');
  return header + rows;
}
