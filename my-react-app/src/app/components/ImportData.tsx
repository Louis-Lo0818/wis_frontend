...
export function ImportData() {
  ...
  const handleProductUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success) {
        let msg = `Successfully imported ${result.importedCount} products`;
        if (result.skippedCount > 0) msg += `, skipped ${result.skippedCount}`;
        setProductStatus('success');
        setProductMessage(msg);
      } else {
        setProductStatus('error');
        setProductMessage(result.errors.join('; ') || 'Import failed');
      }
    } catch (error: unknown) {
      setProductStatus('error');
      setProductMessage(error instanceof Error ? error.message : 'Error uploading CSV file');
    }
  };
  ...
  const handleInventoryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success) {
        let msg = `Successfully imported ${result.importedCount} inventory items`;
        if (result.skippedCount > 0) msg += `, skipped ${result.skippedCount}`;
        setInventoryStatus('success');
        setInventoryMessage(msg);
      } else {
        setInventoryStatus('error');
        setInventoryMessage(result.errors.join('; ') || 'Import failed');
      }
    } catch (error: unknown) {
      setInventoryStatus('error');
      setInventoryMessage(error instanceof Error ? error.message : 'Error uploading CSV file');
    }
  };
