...
import { getProducts, getLocations, transferInventory, getApiErrorMessage, getInventoryLevels } from '../lib/api';
import type { Product, InventoryLevel } from '../types';
...
export function TransferInventory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (qty > availableQuantity) {
      setStatus('error');
      setMessage(`Insufficient quantity. Available: ${availableQuantity.toLocaleString()} units`);
      return;
    }
  ...
  const getAvailableQuantity = (): number => {
    ...
    const inventoryLevel = inventory.find((i) => i.productCode === productCode);
    if (!inventoryLevel) return 0;
    const location = inventoryLevel.locations.find((l) => l.location === fromLocation);
              ...
              <div>
                ...
                {fromLocation && (
                  <p className="mt-2 text-sm text-gray-600">
                    Available: <span className="font-medium">{availableQuantity.toLocaleString()}</span> units at {fromLocation}
                  </p>
                )}
              </div>
